frappe.db.get_single_value("Core Business Cycle Settings","ts_purchase_invoice").then(value =>{
    if(value==1){
		cur_frm.set_df_property("ts_landed_cost_voucher","hidden",0)
		var main_data
		...
		frappe.ui.form.on("TS Landed Cost Voucher",{
			...
			ts_account:function(frm,cdt,cdn){
				...
				if(ts_account_type[0]=="Creditors"){
					frappe.model.set_value(cdt, cdn, "ts_party_type", "Supplier")
				}
		...
		cur_frm.set_df_property("ts_landed_cost_voucher","hidden",1)
	}
})